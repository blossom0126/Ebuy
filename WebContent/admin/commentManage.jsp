...
<script type="text/javascript">
	...
	function formatReplyContent(val, row) {
		if (val == "") {
			return "<a href='javascript:openCommentReplyDialog(" + row.id
					+ ")'>回复</a>";
		} else {
			return val;
		}
	...
	function openCommentReplyDialog(id) {
		$.post("comment_loadCommentById.action", {
			commentId : id
		}, function(result) {
			$("#id").val(result.id);
...
<body style="margin: 1px;">
	<table id="dg" title="留言管理" class="easyui-datagrid" fitColumns="true"
		pagination="true" rownumbers="true" url="comment_listComment.action"
		fit="true" toolbar="#tb">
		<thead>
			<tr>
				...
				<th field="replyContent" width="200" formatter="formatReplyContent"
					align="center">回复内容</th>
	...
	<div id="tb">
		<div>
			<a href="javascript:deleteComment()" class="easyui-linkbutton"
				iconCls="icon-remove" plain="true">删除留言</a>
		</div>
		<div>
			&nbsp;留言名称：&nbsp;<input type="text" id="s_CommentContent" size="20"
				onkeydown="if(event.keyCode==13) searchComment()" /> <a
			...
			<table cellspacing="8px">
				<tr>
					...
					<td colspan="4"><textarea rows="4" cols="40" id="content"
							name="comment.content" readonly="readonly"></textarea></td>
				</tr>
				<tr>
					<td valign="top">回复内容：</td>
					<td colspan="4"><textarea rows="4" cols="40" id="replyContent"
							name="comment.replyContent" class="easyui-validatebox"
							required="true"></textarea> <input type="hidden" id="id"
						name="comment.id" /> <input type="hidden" id="nickName"
						name="comment.nickName" /> <input type="hidden" id="createTime"
						name="comment.createTime" /></td>
				</tr>
			</table>
		</form>
	</div>
